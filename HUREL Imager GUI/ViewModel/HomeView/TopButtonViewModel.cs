    ...
    public class TopButtonViewModel : ViewModelBase
    {
        ...
        private async Task StartSession()
        {
            if (bClicked == false)
            {
                ...
                // 객체탐지 모드일 경우 ObjectDetectionService 초기화
        ...
        private void SaveObjectDetectionData()
        {
            ...
            try
            {
                ...
                if (isTestMode)
                {
                    // 테스트 모드: MyDocuments 폴더 사용
                    directoryPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                    logger.Info($"테스트 모드: 객체탐지 데이터 저장 경로 = {directoryPath}");
                }
                else
                {
                    // 일반 모드: 측정 데이터 폴더 사용
                    directoryPath = Path.GetDirectoryName(LahgiApi.GetFileSavePath());
                    if (string.IsNullOrEmpty(directoryPath))
                    {
                        directoryPath = System.Windows.Forms.Application.StartupPath;
                    }
