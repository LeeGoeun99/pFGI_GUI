...
namespace HUREL
{
	namespace Compton
	{
		...
		public:
			...
			cv::Mat GetCurrentVideoFrame1(bool bCopy = false);	//240104 - SetCurrentVideoframe에서 저장한 영상 획득 //240312 B : bCopy = true(영상 복사)
			cv::Mat GetCurrentVideoFrame2();	//240312 B - Pointcloud Radiationimage 실행 시 설정한 영상 획득
			void	SetCurrentDepthFrame();		//240104 - Depth set
			cv::Mat GetCurrentDepthFrame();
			cv::Mat GetCurrentDepthFrame1();	//240104 - SetCurrentDepthFrame에서 저장한 영상 획득
